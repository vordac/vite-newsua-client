...
const ListItemPublished = ({ article, refreshRate, setRefreshRate }) => {
    const navigate = useNavigate();

    const { title, author, category, views, publishTime, imageUrl, rating } = article;
    const [statusPublished, setStatusPublished] = useState(article.status);
        ...
        <Card className="list-item-news-item">
            <Card.Body className="list-item-news-item-body">
                ...
                <div className='list-item-news-item-body-right'>
                    <select value={statusPublished} onChange={(e) => setStatusPublished(e.target.value)}>
...
export default ListItemPublished;
